...
/** Save TD auth data (token, companyId, userId) */
function saveTDToken(data) {
...
/** Get stored TD auth data (returns null if missing) */
function getTDToken() {
 ...
 * Step 1: If we have a stored token, try using it directly.
 * Step 2: If no token or it fails, login to get a new token.
 * Step 3: Fetch today's time worked stats.
 *
 * @param {object} strategy - One of TD_STRATEGIES
 * @param {string} email
 * @param {string} password
 * @param {object|null} storedAuth - Previously stored token data
 * @returns {Promise<{timeWorked: number, formatted: string, strategy: string, auth: object}>}
 */
async function tryTDStrategy(strategy, email, password, storedAuth) {
    ...
    if (!auth) {
        ...
        // TD API can return: { data: { token, user: {id}, companies: [{id}] } }
        // or: { data: { token, userId, companyId } }
        const d = loginData.data || loginData;
        const token = d.token || (d.user && d.user.token);

        // Company ID: may be nested in companies array or company object
    ...
    // Step 3: Fetch today's time worked — try multiple API endpoints
    ...
    // --- Attempt A: /activity/worklog (returns array of tracked time ranges) ---
    ...
    if (worklogRes.ok) {
        ...
        console.log(tag, 'Worklog response:', JSON.stringify(worklogData).substring(0, 800));

        // Response is { data: [ [ {start, time, ...}, ... ] ] } — nested array
        // Flatten: data may be [[entries]] or [entries]
        let entries = [];
        if (worklogData.data && Array.isArray(worklogData.data)) {
            worklogData.data.forEach(function(item) {
                if (Array.isArray(item)) {
                    entries = entries.concat(item);   // flatten nested array
        ...
        console.log(tag, 'Worklog entries count:', entries.length);

        // Sum the "time" field (tracked seconds) from each entry
        entries.forEach(function(entry) {
            if (entry.time && typeof entry.time === 'number') {
                ...
                totalSeconds += Math.floor((new Date(entry.end) - new Date(entry.start)) / 1000);
            }
        });

        console.log(tag, 'Worklog total seconds:', totalSeconds);
    }

    // --- Attempt B: if worklog gave 0, try /stats (with period param) ---
    if (totalSeconds === 0) {
        ...
        console.log(tag, 'GET stats (period=days) for', dateStr);
        const statsRes = await fetch(statsUrl, { headers: authHeaders });
        console.log(tag, 'Stats response:', statsRes.status, statsRes.statusText);

        if (statsRes.ok) {
            const statsData = await statsRes.json();
            console.log(tag, 'Stats response:', JSON.stringify(statsData).substring(0, 800));

            if (statsData.data && Array.isArray(statsData.data)) {
                statsData.data.forEach(function(item) {
                    const t = item.total || item.totalTracked || item.tracked ||
                              item.totalSec || item.trackedSec || 0;
                    if (t > 0) totalSeconds += t;

                    // Handle nested dates array
                    if (item.dates && Array.isArray(item.dates)) {
                        item.dates.forEach(function(d) {
                            totalSeconds += d.total || d.totalTracked || d.tracked || 0;
                        });
                    }
                });
            } else if (statsData.data && typeof statsData.data === 'object') {
                totalSeconds = statsData.data.total || statsData.data.totalTracked || statsData.data.tracked || 0;
            }
            console.log(tag, 'Stats total seconds:', totalSeconds);
        }
    }

    // --- Attempt C: if still 0, try /activity/summary ---
    if (totalSeconds === 0) {
        const summaryUrl = base + '/activity/summary' +
            '?company=' + auth.companyId +
            '&user=' + auth.userId +
            '&from=' + dateStr +
            '&to=' + dateStr +
        ...
        console.log(tag, 'GET activity/summary for', dateStr);
        const summaryRes = await fetch(summaryUrl, { headers: authHeaders });
        console.log(tag, 'Summary response:', summaryRes.status, summaryRes.statusText);

        if (summaryRes.ok) {
            const summaryData = await summaryRes.json();
            console.log(tag, 'Summary response:', JSON.stringify(summaryData).substring(0, 800));

            if (summaryData.data && Array.isArray(summaryData.data)) {
                summaryData.data.forEach(function(item) {
                    totalSeconds += item.total || item.totalTracked || item.tracked ||
                                    item.totalSec || item.trackedSec || item.duration || 0;
                });
            } else if (summaryData.data && typeof summaryData.data === 'object') {
                totalSeconds = summaryData.data.total || summaryData.data.totalTracked || 0;
            }
            console.log(tag, 'Summary total seconds:', totalSeconds);
        }
    }

    console.log(tag, 'FINAL total seconds worked:', totalSeconds);

    const formatted = secondsToHMS(totalSeconds);
    console.log(tag, 'SUCCESS — Time worked:', formatted);

    return { timeWorked: totalSeconds, formatted: formatted, strategy: strategy.name, auth: auth };
}
...
async function fetchTDData() {
    ...
    for (let i = 0; i < TD_STRATEGIES.length; i++) {
        ...
        try {
            ...
            // Success — show result
            const valueEl = document.getElementById('tdTimeValue');
            const container = document.getElementById('tdResultContainer');
            if (valueEl) valueEl.textContent = result.formatted;
            if (container) container.classList.remove('hidden');

            showTDStatus(
                'Time worked today: ' + result.formatted + ' (via ' + result.strategy + ')',
                'success'
            );
...
/**
 * Apply TimeDoctor time worked to the Elapsed Time input
 */
function applyTDTime() {
    const valueEl = document.getElementById('tdTimeValue');
    if (!valueEl || valueEl.textContent === '--:--:--') {
        showTDStatus('Fetch time worked first.', 'error');
        return;
    }
    ...
    elapsedInput.value = valueEl.textContent;
    elapsedInput.dispatchEvent(new Event('input', { bubbles: true }));

    showTDStatus('Elapsed time set to ' + valueEl.textContent, 'success');
